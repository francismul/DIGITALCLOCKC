...
## [v3.0.0] - 2025-12-10 (Retirement Edition)

### Added

- **Modern UI Refresh** (`src/ui/`)

  - Switched from raw platform APIs to **Nuklear** immediate-mode GUI library.
  - Unified look and feel across Windows and Linux with dedicated large-format clock typography and italic footer styling.
  - Responsive window resizing with scrollable content areas.

- **Modular Architecture**

  - **Core Layer** (`src/core/`): Separated business logic (Time, Config) from UI.
  - **Platform Layer** (`src/platform/`): Abstracted OS-specific system calls (Sleep, Paths).
  - **Plugin System** (`src/plugins/`): Extensible architecture with a sample **Stopwatch** plugin.

- **Configuration System** (`src/core/config.c`)

  - Persistent settings via `~/.digitalclockc.conf` (Linux) or `%USERPROFILE%` (Windows).
  - Configurable 12/24-hour format and date visibility.

- **Power Efficiency**
  - Adaptive main loop with intelligent sleeping (approx. 30Hz) to reduce CPU usage.

### Changed

- **Project Structure**: Reorganized source code into `src/` and `vendor/` directories.
- **Build System**: Updated `Makefile` with automatic OS detection and suppressed vendor warnings.
- **Documentation**: Updated `README.md` to reflect the project's journey and retirement status.

### Deprecated

- **Legacy GUIs**: Removed standalone `digital_clock_gtk.c` and `digital_clock_win32.c` in favor of the unified Nuklear implementation.
- **Legacy Script**: Removed `setup_gtk.sh`.

### Retirement

- This is the final major update. The project is now in maintenance mode/archived status. It serves as a reference for C application evolution from terminal to modular GUI.
